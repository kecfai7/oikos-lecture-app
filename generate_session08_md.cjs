...
fileContent += '\nmodule.exports = { SESSIONS, SLIDES_SESSION_1, SLIDES_SESSION_2, SLIDES_SESSION_3, SLIDES_SESSION_4, SLIDES_SESSION_5, SLIDES_SESSION_6, SLIDES_SESSION_7, SLIDES_SESSION_8 };';

// Write temp file to require
const tempPath = path.join(__dirname, 'temp_slides_data8.cjs');
fs.writeFileSync(tempPath, fileContent, 'utf8');

const { SESSIONS, SLIDES_SESSION_8 } = require(tempPath);
...
console.log(`Loaded ${SLIDES_SESSION_8.length} slides for Session 8.`);

let mdContent = `# Session 8: Agentic Commerce: Human-Not-Present Payments, UCP & AP2 Autonomous Checkout
...
SLIDES_SESSION_8.forEach((slide) => {
...
const outputPath1 = path.join(__dirname, 'session08.md');
const outputPath2 = path.join(__dirname, 'files', 'session08.md');
...
console.log('Generated session08.md successfully!');
