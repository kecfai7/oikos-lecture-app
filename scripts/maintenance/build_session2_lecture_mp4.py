# -*- coding: utf-8 -*-
"""
Oikos University - Session 2 Trio Master Video & Subtitle Generator (MP4 Master Pipeline)
- Presenter 1: Prof. Peter Kim (54) -> Microsoft Neural Voice 'en-US-ChristopherNeural' (Morphed)
- Presenter 2: TA Sarah Jenkins (31) -> Microsoft Neural Voice 'en-US-JennyNeural'
- Presenter 3: TA James Wilson (28) -> Microsoft Neural Voice 'en-US-GuyNeural'
- Output Directory: c:\Oikos Univ\session2_videos
- Features:
  1. High-Resolution 1080p Slide Capture from Web App (Session 2)
  2. Neural Voice Trio Conversational Synthesis with realistic timing pauses
  3. SRT Subtitle Generation with accurate sentence-level timestamps
  4. 1080p MP4 Video Rendering with Embedded Permanent Subtitles
  5. 4x Modular Part Videos + Full 75-Min Master Video Concatenation
"""
...
OUTPUT_DIR = os.path.join(BASE_DIR, "session2_videos")
...
# Voice Configs
VOICE_PETER = "en-US-ChristopherNeural"  # Mature, authoritative professor tone (Morphed with clean_*.wav)
VOICE_PETER_RATE = "-2%"
VOICE_PETER_PITCH = "-2Hz"

VOICE_SARAH = "en-US-JennyNeural"        # Bright, articulate, smart Senior TA tone
VOICE_SARAH_RATE = "+2%"
VOICE_SARAH_PITCH = "+1Hz"

VOICE_JAMES = "en-US-GuyNeural"          # Energetic, sharp, knowledgeable DevOps TA tone
VOICE_JAMES_RATE = "+3%"
...
async def generate_turn_audio(turn_idx, turn_data, slide_num):
    ...
    # If speaker is Professor Peter Kim, apply RVC voice morphing
    if "Peter" in turn_data["speaker"]:
        ...
        # Save TA Sarah or TA James directly
        communicate = edge_tts.Communicate(
            text=turn_data["text"],
            voice=turn_data["voice"],
            rate=turn_data["rate"],
...
async def capture_slide_image(slide_num, page, session_id=2):
...
async def build_slide_video(slide_data, page=None, session_id=2):
    ...
    # 1. Synthesize audio with natural lecture pacing
    turn_audio_files = []
    srt_entries = []
    ...
    # Determine post-slide reflection pause
    if slide_num in [2, 12, 23, 30]:
        outro_pause_sec = 6.0  # Section Introduction Pause
    elif slide_num in [8, 15, 36]:
        outro_pause_sec = 8.0  # Interactive Discussion / Thinking Time
    elif slide_num == 45:
        outro_pause_sec = 10.0 # Final Lab Conclusion Pause
    else:
        outro_pause_sec = 4.0  # Standard Slide Review Pause
    ...
    if page:
        ...
        if not os.path.exists(img_path):
            cmd = [
                ...
                "-i", "color=c=0x0B132B:s=1920x1080:d=1",
                "-vframes", "1", img_path
            ]
    ...
    subtitle_filter = f"subtitles='{clean_srt_name}':force_style='FontSize=16,PrimaryColour=&H00FFFFFF,OutlineColour=&H00000000,BackColour=&H80000000,BorderStyle=4,MarginV=26'"
    ...
    subprocess.run(cmd_encode, cwd=OUTPUT_DIR, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
...
async def build_master_concatenation(video_paths=None, session_id=2, total_slides=45):
    if not video_paths:
        video_paths = []
    ...
    # 1. 4x Modular Part Videos for Session 2:
    # - Part 1: Slides 01 ~ 11 (The 24/7 Sleep-Free Guardian Paradigm)
    # - Part 2: Slides 12 ~ 22 (Under the Hood of the Asynchronous Engine)
    # - Part 3: Slides 23 ~ 29 (The Connected Workspace: Native Cross-App Workflows)
    # - Part 4: Slides 30 ~ 45 (Securing the Digital Vault: Governance & Safety)
    parts_config = [
        ("Part1_Guardian_Paradigm", list(range(1, 12)), "Slides 01~11: The Guardian Paradigm"),
        ("Part2_Async_Engine", list(range(12, 23)), "Slides 12~22: Under the Hood of Async Engine"),
...
def load_session_slides(session_id=2):
...
async def main():
    parser = argparse.ArgumentParser(description="Generate Session 2 Trio Master Lecture MP4 Videos")
    parser.add_argument("--session", type=int, default=2, help="Session number (default: 2)")
    parser.add_argument("--slide", type=int, help="Generate video for a single slide number (e.g. --slide 1)")
    parser.add_argument("--slides", type=str, help="Comma-separated slide numbers (e.g. --slides 1,11,22,45)")
    ...
    if args.slide:
        ...
        # Default: generate demo slides (Slide 1 & Slide 11)
        target_slides = [s for s in session_slides if s["num"] in [1, 11]]

    print("=======================================================")
    print(f"🚀 Oikos Univ 3-Presenter Trio Master Video Generator (Session {args.session})")
