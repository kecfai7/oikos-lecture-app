...
fileContent += '\nmodule.exports = { SESSIONS, SLIDES_SESSION_1, SLIDES_SESSION_2, SLIDES_SESSION_3 };';

// Write temp file to require
const tempPath = path.join(__dirname, 'temp_slides_data3.cjs');
fs.writeFileSync(tempPath, fileContent, 'utf8');

const { SESSIONS, SLIDES_SESSION_3 } = require(tempPath);
...
console.log(`Loaded ${SLIDES_SESSION_3.length} slides for Session 3.`);

let mdContent = `# Session 3: The Battle for the OS Shell: Windows Dominance and the 1.2GB Trojan Horse
...
SLIDES_SESSION_3.forEach((slide) => {
...
const outputPath1 = path.join(__dirname, 'session03.md');
const outputPath2 = path.join(__dirname, 'files', 'session03.md');
...
console.log('Generated session03.md successfully!');
