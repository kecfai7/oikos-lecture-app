...
fileContent += '\nmodule.exports = { SESSIONS, SLIDES_SESSION_1, SLIDES_SESSION_2, SLIDES_SESSION_3, SLIDES_SESSION_4, SLIDES_SESSION_5, SLIDES_SESSION_6, SLIDES_SESSION_7, SLIDES_SESSION_8, SLIDES_SESSION_9, SLIDES_SESSION_10, SLIDES_SESSION_11 };';

// Write temp file to require
const tempPath = path.join(__dirname, 'temp_slides_data11.cjs');
fs.writeFileSync(tempPath, fileContent, 'utf8');

const { SESSIONS, SLIDES_SESSION_11 } = require(tempPath);
...
console.log(`Loaded ${SLIDES_SESSION_11.length} slides for Session 11.`);

let mdContent = `# Session 11: True AI Science: HeurekaBench & Fact Verification (THINK-ACT-OBSERVE) Governance
...
SLIDES_SESSION_11.forEach((slide) => {
...
const outputPath1 = path.join(__dirname, 'session11.md');
const outputPath2 = path.join(__dirname, 'files', 'session11.md');
...
console.log('Generated session11.md successfully!');
