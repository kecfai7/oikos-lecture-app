...
fileContent += '\nmodule.exports = { SESSIONS, SLIDES_SESSION_1, SLIDES_SESSION_2, SLIDES_SESSION_3, SLIDES_SESSION_4, SLIDES_SESSION_5, SLIDES_SESSION_6 };';

// Write temp file to require
const tempPath = path.join(__dirname, 'temp_slides_data6.cjs');
fs.writeFileSync(tempPath, fileContent, 'utf8');

const { SESSIONS, SLIDES_SESSION_6 } = require(tempPath);
...
console.log(`Loaded ${SLIDES_SESSION_6.length} slides for Session 6.`);

let mdContent = `# Session 6: The 1-Million Token Playground: Vibe Coding, Many-shot ICL, and Cost Optimization with Google AI Studio
...
SLIDES_SESSION_6.forEach((slide) => {
...
const outputPath1 = path.join(__dirname, 'session06.md');
const outputPath2 = path.join(__dirname, 'files', 'session06.md');
...
console.log('Generated session06.md successfully!');
