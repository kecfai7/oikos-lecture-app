...
fileContent += '\nmodule.exports = { SESSIONS, SLIDES_SESSION_1, SLIDES_SESSION_2, SLIDES_SESSION_3, SLIDES_SESSION_4, SLIDES_SESSION_5, SLIDES_SESSION_6, SLIDES_SESSION_7, SLIDES_SESSION_8, SLIDES_SESSION_9, SLIDES_SESSION_10 };';

// Write temp file to require
const tempPath = path.join(__dirname, 'temp_slides_data10.cjs');
fs.writeFileSync(tempPath, fileContent, 'utf8');

const { SESSIONS, SLIDES_SESSION_10 } = require(tempPath);
...
console.log(`Loaded ${SLIDES_SESSION_10.length} slides for Session 10.`);

let mdContent = `# Session 10: Escaping Developer Gravity: Antigravity 2.0 & Multi-Agent Orchestration Blueprint
...
SLIDES_SESSION_10.forEach((slide) => {
...
const outputPath1 = path.join(__dirname, 'session10.md');
const outputPath2 = path.join(__dirname, 'files', 'session10.md');
...
console.log('Generated session10.md successfully!');
