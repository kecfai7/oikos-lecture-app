...
fileContent += '\nmodule.exports = { SESSIONS, SLIDES_SESSION_1, SLIDES_SESSION_2, SLIDES_SESSION_3, SLIDES_SESSION_4 };';

// Write temp file to require
const tempPath = path.join(__dirname, 'temp_slides_data4.cjs');
fs.writeFileSync(tempPath, fileContent, 'utf8');

const { SESSIONS, SLIDES_SESSION_4 } = require(tempPath);
...
console.log(`Loaded ${SLIDES_SESSION_4.length} slides for Session 4.`);

let mdContent = `# Session 4: Grounded Intelligence on My Data: The RAG Revolution and Private Knowledge Factories
...
SLIDES_SESSION_4.forEach((slide) => {
...
const outputPath1 = path.join(__dirname, 'session04.md');
const outputPath2 = path.join(__dirname, 'files', 'session04.md');
...
console.log('Generated session04.md successfully!');
