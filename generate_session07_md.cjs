...
fileContent += '\nmodule.exports = { SESSIONS, SLIDES_SESSION_1, SLIDES_SESSION_2, SLIDES_SESSION_3, SLIDES_SESSION_4, SLIDES_SESSION_5, SLIDES_SESSION_6, SLIDES_SESSION_7 };';

// Write temp file to require
const tempPath = path.join(__dirname, 'temp_slides_data7.cjs');
fs.writeFileSync(tempPath, fileContent, 'utf8');

const { SESSIONS, SLIDES_SESSION_7 } = require(tempPath);
...
console.log(`Loaded ${SLIDES_SESSION_7.length} slides for Session 7.`);

let mdContent = `# Session 7: The New Map of the AI-Native Web: Escaping the HTML Maze and Token Diet with WebMCP Protocol
...
SLIDES_SESSION_7.forEach((slide) => {
...
const outputPath1 = path.join(__dirname, 'session07.md');
const outputPath2 = path.join(__dirname, 'files', 'session07.md');
...
console.log('Generated session07.md successfully!');
