...
fileContent += '\nmodule.exports = { SESSIONS, SLIDES_SESSION_1, SLIDES_SESSION_2, SLIDES_SESSION_3, SLIDES_SESSION_4, SLIDES_SESSION_5 };';

// Write temp file to require
const tempPath = path.join(__dirname, 'temp_slides_data5.cjs');
fs.writeFileSync(tempPath, fileContent, 'utf8');

const { SESSIONS, SLIDES_SESSION_5 } = require(tempPath);
...
console.log(`Loaded ${SLIDES_SESSION_5.length} slides for Session 5.`);

let mdContent = `# Session 5: From Personal Drawer to System Vault: Enterprise Google Drive Mastery, Apps Script Automation & Governance
...
SLIDES_SESSION_5.forEach((slide) => {
...
const outputPath1 = path.join(__dirname, 'session05.md');
const outputPath2 = path.join(__dirname, 'files', 'session05.md');
...
console.log('Generated session05.md successfully!');
