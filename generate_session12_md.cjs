...
fileContent += '\nmodule.exports = { SESSIONS, SLIDES_SESSION_1, SLIDES_SESSION_2, SLIDES_SESSION_3, SLIDES_SESSION_4, SLIDES_SESSION_5, SLIDES_SESSION_6, SLIDES_SESSION_7, SLIDES_SESSION_8, SLIDES_SESSION_9, SLIDES_SESSION_10, SLIDES_SESSION_11, SLIDES_SESSION_12 };';

// Write temp file to require
const tempPath = path.join(__dirname, 'temp_slides_data12.cjs');
fs.writeFileSync(tempPath, fileContent, 'utf8');

const { SESSIONS, SLIDES_SESSION_12 } = require(tempPath);
...
console.log(`Loaded ${SLIDES_SESSION_12.length} slides for Session 12.`);

let mdContent = `# Session 12: World Models: Genie 3 Simulation & Waymo Autonomous Training
...
SLIDES_SESSION_12.forEach((slide) => {
...
const outputPath1 = path.join(__dirname, 'session12.md');
const outputPath2 = path.join(__dirname, 'files', 'session12.md');
...
console.log('Generated session12.md successfully!');
